...
- Read from the backlog following `${CLAUDE_PLUGIN_ROOT}/reference/datastore.md`, scoping every query to this repository's product — the binding in `.superdev/product.json` says which one, and a shared backlog will happily hand you another product's Capabilities if you don't
...
- **If there are no Capabilities at all**, the backlog was never initialized — stop and use superdev:init rather than inventing a first Capability here.  Seeding the backlog is a decision the user makes deliberately, not a side effect of planning one idea.
...
- Write the validated Features and Acceptance Criteria to the backlog, following `${CLAUDE_PLUGIN_ROOT}/reference/datastore.md`, along with their edges to the Capabilities you identified
...
- You're about to write a record whose shape the backlog can't store
