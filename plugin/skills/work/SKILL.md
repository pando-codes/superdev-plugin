...
2. **Count the backlog namespaces you hold, before you call anything.**

   - Exactly one `mcp__backlog-<role>__*` namespace and no `mcp__backlog__*`: you are that
     role's pinned agent. Continue.
   - More than one `mcp__backlog-*__` namespace, or `mcp__backlog__*` at all: you are the main
     session, or an agent that inherited a session's surface (superdev issue #73). Work
     through `mcp__backlog__*` as `<label>-main` and **do not choose a pinned server** —
     picking `backlog-engineer` from a menu of four is choosing a role, which is the one thing
     no caller may do. The unpinned server's role is decided by the credential, not by you.
   - You were dispatched *as* a role and hold several namespaces: say so and stop. You are
     not that agent, and finishing its work under its `agent_id` puts a name on the record
     that did not act.

3. **Call `backlog_whoami`.** It answers three things you need before doing anything:
...
4. **If several agents share this session,** pass a distinct `agent_id` on every work tool
