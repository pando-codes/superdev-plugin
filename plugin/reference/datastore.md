...
Use this reference whenever a skill reads from or writes to the backlog.  The other reference
...
store isn't in the backlog, and a half-written plan scattered across the working tree is worse
...
writes that product and refuses every other, and **reads only that product's backlog** —
...
`backlog_whoami` reports it, and is worth calling once at the start of a session
...
- **A refusal names 42501** and is not about your role. If `backlog_whoami` says
...
- **Another product's records read as absent, not forbidden.** A capability list for a
  ...
  That is the scope, not a bad key — `backlog_whoami` is how you tell the difference.
...
route around it.  Call **`backlog_whoami`** when a write is unexpectedly refused; it reports the
...
A backlog holds many products, and **every entity names its product directly** — `capability`,
...
Read it before any other backlog access and pass that `product_key` to every call that takes
...
there.  If it names a product the backlog does not hold, stop and ask; do not create it, because
...
| `backlog_get_feature` | one feature with its stories, criteria, and verified state |
| `backlog_get_story` | one story with its derived confidence |
| `backlog_get_acceptance_criterion` | one criterion with its latest evaluation |
| `backlog_coverage` | the product's weighted verified share |
| `backlog_model_health` | problems the model detects in itself |
| `backlog_public_view` | the GTM projection — public capabilities only |
| `backlog_list_work` | the work queue, every role's, with a `ready` flag |
| `backlog_get_work` | one work item's full brief |
...
`backlog_model_health` is the first thing to check when a figure looks wrong.
...
| `backlog_create_acceptance_criterion`, `backlog_update_acceptance_criterion` | product-manager |
| `backlog_link`, `backlog_unlink` | product-manager |
| `backlog_record_evaluation` | quality-assurance or ci |
| `backlog_record_evidence` | revops or ci |
| `backlog_claim_work`, `backlog_heartbeat_work`, `backlog_push_progress`, `backlog_finish_work` | any role, for work addressed to it |
| `backlog_file_work`, `backlog_steward_work` | product-manager or head-of-engineering |
...
Every entity has an `id` the backlog generates and never hands you in advance.  You address rows
...
`backlog_link` and `backlog_unlink` take a `kind` naming which two things to join:
...
backlog_link kind=work-item-feature  work_item_key=wi_a1b2c3
             product_key=reelmates   feature_key=cost-attribution
backlog_link kind=work-item-ac       work_item_key=wi_a1b2c3  ac_key=ac_7bq1lm
```
...
```
backlog://schema/product      backlog://schema/capability   backlog://schema/feature
backlog://schema/story        backlog://schema/acceptance-criterion
```
...
The reference docs in this folder describe a richer model than the backlog currently stores.
...
| Described in reference | State in the backlog |
|---|---|
...
Everything above describes what the backlog *records*. This describes what it *hands out*.
...
```
backlog_claim_work  product_key=reelmates  lease_seconds=900
```
...
**3. A claim is a LEASE, not an assignment.** It expires. `backlog_heartbeat_work` extends it;
...
leaving room on both sides. `ready` in `backlog_list_work` is the same three-part condition a
...
That payload is why pulling work from the backlog beats being told what to do in prose. If it
...
`backlog_file_work` needs product-manager or head-of-engineering, and the database enforces
**shape, not quality**: an intent reading "improve things" satisfies every constraint and is
worthless to whoever claims it.

Then **link it** — `backlog_link` with `kind='work-item-feature'` and `kind='work-item-ac'` is
what puts the stories and criteria into the brief. An unlinked work item hands an agent one
sentence and no criteria.

### Notes are append-only

`backlog_push_progress` writes a permanent row; there is no edit and no delete. `decision` is
...
`backlog_finish_work state=done` says you did the work. It does not say the criteria pass —
that is `backlog_record_evaluation`, and it is deliberately unavailable to the role that builds.
